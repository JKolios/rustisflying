...
- `src/output/epaper/` — Waveshare 2.7inch e-Paper HAT (B) tri-color panel. `layout.rs` (pure wording, unit-tested) and `render.rs` (two 1-bit planes via epd-waveshare + embedded-graphics + profont) always compile; `hw.rs` (SPI/GPIO worker thread) is gated behind `--features epaper`, Linux only. Rendering choices forced by the hardware: the crate's `Display2in7b` framebuffer is black/white-only, so red accents go into a second "chromatic" framebuffer; profont is latin1-only, so arrows are drawn as line/triangle shapes. **Framebuffer polarity (verified on hardware):** the two planes have opposite polarity — for the black plane a SET buffer bit is black ink and a CLEARED bit is white paper; for the chromatic plane a CLEARED bit is red and a SET bit is "no red" (the driver byte-inverts both planes on the wire). The `INK`/`PAPER` and `CHROMATIC_INK`/`CHROMATIC_PAPER` constants in `render.rs` encode this; don't "fix" the Color names. Use `render::blank()`, not the driver's `clear_frame`, for an all-white start. The worker skips the refresh when the frame is byte-identical — e-paper refreshes are slow and wear the panel.
